...
export async function POST(request: Request) {
  ...
  const { data: brand } = await supabase.from("brands").select("user_id").eq("id", brand_id).single()
  if (!brand || brand.user_id !== user.id) return NextResponse.json(buildError(ErrorCodes.BRAND_NOT_FOUND, "Brand not found."), { status: 404 })
...
export async function DELETE(request: Request) {
  ...
  const { data: entry } = await supabase.from("calendar_entries").select("brand_id").eq("id", entryId).single()
  if (!entry) return NextResponse.json(buildError(ErrorCodes.NOT_FOUND, "Entry not found."), { status: 404 })

  const { data: brand } = await supabase.from("brands").select("user_id").eq("id", entry.brand_id).single()
