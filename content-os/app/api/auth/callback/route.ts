        ...
        if (user) {
          ...
          if (isNewUser) {
            ...
            if (user.email) {
              ...
              await sendWelcomeEmail(user.id, user.email, fullName).catch((err) =>
                console.error("[auth/callback] sendWelcomeEmail failed for user", user.id, err instanceof Error ? err.message : err)
              )
            }
          }
        }
