...
type Params = { params: Promise<{ brandId: string; postId: string }> }
...
export async function PUT(request: Request, { params }: Params) {
  console.log("[brands/blog-posts/:id] PUT called")

  let supabase
  try {
    supabase = await createClient()
  } catch (err) {
    console.error("[brands/blog-posts/:id] createClient failed:", err)
  ...
  try {
    ...
    const { data, error } = await (supabase.from("blog_posts") as any)
      ...
      .eq("brand_id", brandId)
      .select()
      .single() as { data: BlogPostRow | null; error: { message: string } | null }
    if (error) {
      console.error("[brands/blog-posts/:id] update error:", error)
      return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Update failed."), { status: 500 })
    }
    return NextResponse.json({ data })
  } catch (err) {
    console.error("[brands/blog-posts/:id] error:", err)
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Update failed."), { status: 500 })
  }
}
