...
type Params = { params: Promise<{ brandId: string; sequenceId: string }> }
...
export async function PUT(request: Request, { params }: Params) {
  console.log("[brands/email-sequences/:id] PUT called")

  let supabase
  try {
    supabase = await createClient()
  } catch (err) {
    console.error("[brands/email-sequences/:id] createClient failed:", err)
  ...
  try {
    ...
    const { data, error } = await (supabase.from("email_sequences") as any)
      ...
      .eq("brand_id", brandId)
      .select()
      .single() as { data: EmailSequenceRow | null; error: { message: string } | null }
    if (error) {
      console.error("[brands/email-sequences/:id] update error:", error)
      return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Update failed."), { status: 500 })
    }
    return NextResponse.json({ data })
  } catch (err) {
    console.error("[brands/email-sequences/:id] error:", err)
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Update failed."), { status: 500 })
  }
}
