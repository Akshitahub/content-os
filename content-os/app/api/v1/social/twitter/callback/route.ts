...
export async function GET(request: Request) {
  console.log("[social/twitter/callback] GET called")
  ...
  if (!brandId) {
    console.error("[social/twitter/callback] missing brandId")
    return NextResponse.redirect(`${appUrl}/dashboard?twitter_error=server_error`)
  }

  if (connected !== "twitter" || !accountId) {
    console.error("[social/twitter/callback] connection did not complete:", { connected, accountId })
    return redirectToBrand(appUrl, brandId, { twitter_error: "oauth_denied" })
  }
  ...
  if (!brand || brand.user_id !== user.id) {
    console.error("[social/twitter/callback] brand not found or not owned by user:", brandId)
    return NextResponse.redirect(`${appUrl}/dashboard?twitter_error=server_error`)
  }
  ...
  if (!PLAN_LIMITS[plan].zernioSocialPlatforms) {
    console.error(`[social/twitter/callback] brand ${brandId}'s plan (${plan}) does not include Zernio social platforms`)
    return redirectToBrand(appUrl, brandId, { twitter_error: "plan_restricted" })
  }
  ...
  if (!profileId) {
    console.error("[social/twitter/callback] no profileId available from redirect or cookie")
    return redirectToBrand(appUrl, brandId, { twitter_error: "server_error" })
  }
  ...
  if (upsertError) {
    console.error("[social/twitter/callback] failed to save connection:", upsertError)
    return redirectToBrand(appUrl, brandId, { twitter_error: "server_error" })
  }

  const response = redirectToBrand(appUrl, brandId, { twitter_success: "1" })
