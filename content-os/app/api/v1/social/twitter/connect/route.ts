...
export async function GET(request: Request) {
  console.log("[social/twitter/connect] GET called")
  ...
  // unified API billed per connected account across our whole Zernio
  ...
  if (!PLAN_LIMITS[plan].zernioSocialPlatforms && !isInternalUnlimited(user.id)) {
    return NextResponse.json(
      buildError(ErrorCodes.USAGE_LIMIT_EXCEEDED, "LinkedIn, YouTube, and Twitter/X publishing are available on Pro and Agency plans. Upgrade to connect this platform."),
      { status: 403 }
    )
  }

  if (!process.env.ZERNIO_API_KEY) {
    console.error("[social/twitter/connect] ZERNIO_API_KEY is not configured")
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Twitter/X connect is not configured."), { status: 500 })
  }

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? origin
  const redirectUri = `${appUrl}/api/v1/social/twitter/callback?brandId=${brandId}`
  ...
  try {
    ...
    if (existing?.zernio_profile_id) {
      ...
      const { error: profileUpsertError } = await socialConnectionsTable(supabase)
        .upsert(
          { brand_id: brandId, platform: "twitter", zernio_profile_id: profileId, access_token: null, token_expires_at: null, is_active: false },
          { onConflict: "brand_id,platform" }
        )
      if (profileUpsertError) {
        console.error("[social/twitter/connect] failed to persist zernio_profile_id:", profileUpsertError)
      }
    }

    const { authUrl } = await getZernioConnectUrl("twitter", profileId, redirectUri)

    const response = NextResponse.redirect(authUrl)
    response.cookies.set("zernio_profile_id", profileId, { maxAge: 600, httpOnly: true, secure: true, sameSite: "lax" })
    return response
  } catch (err) {
    console.error("[social/twitter/connect] failed to start connect flow:", err instanceof Error ? err.message : err)
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Couldn't start the Twitter/X connect flow. Please try again."), { status: 500 })
  }
}
