  ...
  try {
    ...
    if (existing?.zernio_profile_id) {
      ...
      const { error: profileUpsertError } = await socialConnectionsTable(supabase)
        .upsert(
          { brand_id: brandId, platform: "instagram", zernio_profile_id: profileId, access_token: null, token_expires_at: null, is_active: false },
          { onConflict: "brand_id,platform" }
        )
      if (profileUpsertError) {
        console.error("[social/instagram/connect] failed to persist zernio_profile_id:", profileUpsertError)
      }
    }
