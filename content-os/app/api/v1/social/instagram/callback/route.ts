...
export async function GET(request: Request) {
  ...
  if (!brandId) {
    console.error("[social/instagram/callback] missing brandId")
    return NextResponse.redirect(`${appUrl}/dashboard?ig_error=server_error`)
  }

  if (connected !== "instagram" || !accountId) {
    console.error("[social/instagram/callback] connection did not complete:", { connected, accountId })
    return redirectToBrand(appUrl, brandId, { ig_error: "oauth_denied" })
  }
  ...
  if (!PLAN_LIMITS[plan].zernioSocialPlatforms && !isInternalUnlimited(user.id)) {
    console.error(`[social/instagram/callback] brand ${brandId}'s plan (${plan}) does not include Zernio social platforms`)
    return redirectToBrand(appUrl, brandId, { ig_error: "plan_restricted" })
  }
  ...
  if (!profileId) {
    console.error("[social/instagram/callback] no profileId available from redirect or cookie")
  ...
  const response = redirectToBrand(appUrl, brandId, { ig_success: "1" })
