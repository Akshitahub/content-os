...
export async function GET(request: Request) {
  ...
  // per-account cost.
  if (!ENABLED_SOCIAL_PLATFORMS.includes("threads")) {
    return NextResponse.json(
      buildError(ErrorCodes.FEATURE_UNAVAILABLE, "Threads isn't available yet — Instagram is the only platform we support right now. More are coming as we grow."),
      { status: 403 }
    )
  }

  // Threads now routes through Zernio like Instagram/Pinterest/LinkedIn/
  // YouTube/Twitter — billed per connected account across our whole Zernio
  // account — gate it to paid plans so a trialing/Starter connection
  ...
  if (!PLAN_LIMITS[plan].zernioSocialPlatforms && !isInternalUnlimited(user.id)) {
    return NextResponse.json(
      buildError(ErrorCodes.USAGE_LIMIT_EXCEEDED, "Threads publishing is available on Pro and Agency plans. Upgrade to connect this platform."),
      { status: 403 }
    )
  }

  if (!process.env.ZERNIO_API_KEY) {
    console.error("[social/threads/connect] ZERNIO_API_KEY is not configured")
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Threads connect is not configured."), { status: 500 })
  }

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? origin
  const redirectUri = `${appUrl}/api/v1/social/threads/callback?brandId=${brandId}`
  ...
  try {
    ...
    if (existing?.zernio_profile_id) {
      ...
      const { error: profileUpsertError } = await socialConnectionsTable(supabase)
        .upsert(
          { brand_id: brandId, platform: "threads", zernio_profile_id: profileId, access_token: null, token_expires_at: null, is_active: false },
          { onConflict: "brand_id,platform" }
        )
      if (profileUpsertError) {
        console.error("[social/threads/connect] failed to persist zernio_profile_id:", profileUpsertError)
      }
    }

    const { authUrl } = await getZernioConnectUrl("threads", profileId, redirectUri)

    const response = NextResponse.redirect(authUrl)
    response.cookies.set("zernio_profile_id", profileId, { maxAge: 600, httpOnly: true, secure: true, sameSite: "lax" })
    return response
  } catch (err) {
    console.error("[social/threads/connect] failed to start connect flow:", err instanceof Error ? err.message : err)
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Couldn't start the Threads connect flow. Please try again."), { status: 500 })
  }
}
