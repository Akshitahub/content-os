...
export async function GET(request: Request) {
  ...
  if (!brandId) {
    console.error("[social/threads/callback] missing brandId")
    return NextResponse.redirect(`${appUrl}/dashboard?threads_error=server_error`)
  }

  if (connected !== "threads" || !accountId) {
    console.error("[social/threads/callback] connection did not complete:", { connected, accountId })
    return redirectToBrand(appUrl, brandId, { threads_error: "oauth_denied" })
  }
  ...
  if (!PLAN_LIMITS[plan].zernioSocialPlatforms && !isInternalUnlimited(user.id)) {
    console.error(`[social/threads/callback] brand ${brandId}'s plan (${plan}) does not include Zernio social platforms`)
    return redirectToBrand(appUrl, brandId, { threads_error: "plan_restricted" })
  }
  ...
  if (!profileId) {
    console.error("[social/threads/callback] no profileId available from redirect or cookie")
  ...
  const response = redirectToBrand(appUrl, brandId, { threads_success: "1" })
