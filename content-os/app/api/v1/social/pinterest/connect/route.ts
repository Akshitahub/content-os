...
export async function GET(request: Request) {
  ...
  // per-account cost.
  if (!ENABLED_SOCIAL_PLATFORMS.includes("pinterest")) {
    return NextResponse.json(
      buildError(ErrorCodes.FEATURE_UNAVAILABLE, "Pinterest isn't available yet — Instagram is the only platform we support right now. More are coming as we grow."),
      { status: 403 }
    )
  }

  // Pinterest now routes through Zernio like Instagram/LinkedIn/YouTube/
  // Twitter — billed per connected account across our whole Zernio account —
  // gate it to paid plans so a trialing/Starter connection doesn't become
  // pure cost with no matching revenue.
  ...
  if (!PLAN_LIMITS[plan].zernioSocialPlatforms && !isInternalUnlimited(user.id)) {
    return NextResponse.json(
      buildError(ErrorCodes.USAGE_LIMIT_EXCEEDED, "Pinterest publishing is available on Pro and Agency plans. Upgrade to connect this platform."),
      { status: 403 }
    )
  }

  if (!process.env.ZERNIO_API_KEY) {
    console.error("[social/pinterest/connect] ZERNIO_API_KEY is not configured")
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Pinterest connect is not configured."), { status: 500 })
  }

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? origin
  const redirectUri = `${appUrl}/api/v1/social/pinterest/callback?brandId=${brandId}`
  ...
  try {
    ...
    console.error("[social/pinterest/connect] failed to start connect flow:", err instanceof Error ? err.message : err)
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Couldn't start the Pinterest connect flow. Please try again."), { status: 500 })
  }
}
