...
export async function GET(request: Request) {
  console.log("[social/youtube/connect] GET called")
  ...
  // per-account cost.
  if (!ENABLED_SOCIAL_PLATFORMS.includes("youtube")) {
    return NextResponse.json(
      buildError(ErrorCodes.FEATURE_UNAVAILABLE, "YouTube isn't available yet — Instagram is the only platform we support right now. More are coming as we grow."),
      { status: 403 }
    )
  }
  ...
  if (!process.env.ZERNIO_API_KEY) {
    console.error("[social/youtube/connect] ZERNIO_API_KEY is not configured")
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "YouTube connect is not configured."), { status: 500 })
  }

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? origin
  const redirectUri = `${appUrl}/api/v1/social/youtube/callback?brandId=${brandId}`
  ...
  try {
    ...
    if (existing?.zernio_profile_id) {
      ...
      const { error: profileUpsertError } = await socialConnectionsTable(supabase)
        .upsert(
          { brand_id: brandId, platform: "youtube", zernio_profile_id: profileId, access_token: null, token_expires_at: null, is_active: false },
          { onConflict: "brand_id,platform" }
        )
      if (profileUpsertError) {
        console.error("[social/youtube/connect] failed to persist zernio_profile_id:", profileUpsertError)
      }
    }

    const { authUrl } = await getZernioConnectUrl("youtube", profileId, redirectUri)

    const response = NextResponse.redirect(authUrl)
    response.cookies.set("zernio_profile_id", profileId, { maxAge: 600, httpOnly: true, secure: true, sameSite: "lax" })
    return response
  } catch (err) {
    console.error("[social/youtube/connect] failed to start connect flow:", err instanceof Error ? err.message : err)
    return NextResponse.json(buildError(ErrorCodes.INTERNAL_ERROR, "Couldn't start the YouTube connect flow. Please try again."), { status: 500 })
  }
}
