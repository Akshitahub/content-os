...
export default async function SettingsPage({
  ...
  const [profileResult, brandsResult] = await Promise.all([
    supabase
      .from("users")
      .select("full_name, email, plan, generation_count, generation_count_reset_at, reel_count_this_week, reel_count_reset_at")
      .eq("id", user.id)
      .single<Pick<UserRow, "full_name" | "email" | "plan" | "generation_count" | "generation_count_reset_at" | "reel_count_this_week" | "reel_count_reset_at">>(),
    supabase
      .from("brands")
