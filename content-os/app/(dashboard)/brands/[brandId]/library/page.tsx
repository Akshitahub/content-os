...
import { Archive, Copy, Check, Star, Sparkles, BookOpen, ChevronDown, ChevronUp, Film, LayoutGrid, Megaphone, Mail, FileText, ImageIcon, Download, Search, Loader2 } from "lucide-react"
...
function GenerateVideoAction({ script, brandId }: { script: ReelScriptRow; brandId: string }) {
  const [jobId, setJobId] = useState<string | null>(null)

  const startMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/v1/brands/${brandId}/reel-scripts/${script.id}/video`, { method: "POST" })
  ...
  const statusQuery = useQuery({
    queryKey: ["reel-video-job", script.id, jobId],
    queryFn: async (): Promise<VideoJobStatusResponse> => {
      const res = await fetch(`/api/v1/brands/${brandId}/reel-scripts/${script.id}/video?jobId=${jobId}`)
    ...
    <div className="space-y-1.5 border-t pt-2">
      ...
      {jobId && job && (
        <div className="text-xs">
          ...
          {job.status === "failed" && (
            <p className="text-destructive">{job.error_message ?? "Video generation failed."}</p>
          )}
        </div>
      )}
  ...
  return (
    <Card className="transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
      ...
      <CardContent className="space-y-3">
        ...
        <GenerateVideoAction script={script} brandId={brandId} />
      </CardContent>
    </Card>
  )
}
