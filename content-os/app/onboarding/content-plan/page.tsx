  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/v1/brands/${brandId}`, {
        ...
        body: JSON.stringify({ posting_frequency: frequency, target_platforms: platforms, onboarding_type: goal }),
      })
      if (!res.ok) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {FREQUENCIES.map(f => (
              <button key={f.id} type="button" onClick={() => setFrequency(f.id)}
                className={`rounded-xl border-2 p-4 text-left transition-all ${frequency === f.id ? "border-violet-500 bg-violet-950/40" : "border-white/10 bg-white/5 hover:border-white/20"}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {PLATFORMS.map(p => (
              <button key={p.id} type="button" onClick={() => togglePlatform(p.id)}
                className={`rounded-full border px-4 py-2 text-sm transition-all ${platforms.includes(p.id) ? "border-violet-500 bg-violet-600/20 text-violet-300" : "border-white/10 bg-white/5 text-gray-400 hover:border-white/20"}`}>
                {p.label}
              </button>
            ))}
          </div>
          {platforms.length === 0 && <p className="mt-2 text-xs text-red-400">Select at least one platform.</p>}
        </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {GOALS.map(g => (
              <button key={g.id} type="button" onClick={() => setGoal(g.id)}
                className={`rounded-xl border-2 p-3 text-left transition-all ${goal === g.id ? "border-violet-500 bg-violet-950/40" : "border-white/10 bg-white/5 hover:border-white/20"}`}>
        ...
        <button type="submit" disabled={saving || platforms.length === 0}
          ...
          {saving ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving…</> : <>Save and continue <ArrowRight className="h-4 w-4" /></>}
        </button>
      </form>
    </div>
  )
