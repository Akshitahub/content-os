...
export default function BrandProfilePage() {
  ...
  useEffect(() => {
    if (!brandId) { router.replace("/onboarding/brand-setup"); return }
    fetch(`/api/v1/brands/${brandId}`)
      .then(r => r.json() as Promise<{ data?: Brand; error?: { message: string } }>)
      .then(json => {
        if (json.data) setBrand(json.data)
        else setError(json.error?.message ?? "Couldn't load your brand profile.")
      })
  ...
  return (
    <div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <p className="text-xs text-gray-600">You can edit all of this in brand settings later.</p>
        <button
