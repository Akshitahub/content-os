...
export function buildCaptionUserPrompt(
  ...
  // Only meaningful when suggested_template is "hard_truth_checklist" --
  // appended onto the same JSON contract every other caller already gets
  // rather than a separate call, so this stays one Groq call regardless of
  // which template gets picked. The model is told explicitly these are
  // conditional (see suggested_template's own instruction above) so it
  // doesn't feel obligated to fill them for an unrelated template.
  const checklistFieldsBlock = `,\n  "checklist_headline": "ONLY if suggested_template is hard_truth_checklist -- a genuine contrarian/myth-busting hook headline, under 10 words, otherwise omit this field entirely",\n  "checklist_highlighted_phrase": "ONLY if suggested_template is hard_truth_checklist -- a short 2-4 word phrase taken VERBATIM from checklist_headline to visually highlight, otherwise omit this field entirely",\n  "checklist_wrong_items": "ONLY if suggested_template is hard_truth_checklist -- an array of 2-3 short 'wrong way' lines (under 8 words each), otherwise omit this field entirely",\n  "checklist_right_items": "ONLY if suggested_template is hard_truth_checklist -- an array of 2-3 short 'right way' lines, each one the direct fix for the wrong item at the same position, otherwise omit this field entirely",\n  "checklist_closing_line": "ONLY if suggested_template is hard_truth_checklist -- one closing line that reframes the point (not a summary, a sharper final statement), otherwise omit this field entirely"`
  const imagePromptField = options.includeImagePrompt
    ? `,\n  "image_prompt": "vivid scene description grounded in this post's specific message/topic above, no text or words in the image, lower third kept visually simpler for text overlay — irrelevant and can be a short placeholder if suggested_template is hard_truth_checklist, since that template has no photo at all",\n  "suggested_template": "one of: bold_statement | product_focus | quote_card | minimal | blank | hard_truth_checklist — pick whichever best fits this post's specific message and mood, e.g. blank for a clean lifestyle photo that speaks for itself, bold_statement for a punchy claim, quote_card for something reflective. Pick hard_truth_checklist specifically when this post is genuinely a myth-busting/contrarian post (challenging a common misconception) or a problem-and-solution post (a clear wrong way vs. right way) — it renders as a flat card with NO photo: a headline, a short 'wrong way' list, a short 'right way' list, and a closing line, so only pick it when the post's actual content naturally splits into that shape. Don't force it onto a post that isn't genuinely a comparison.",\n  "suggested_color_theme_id": "one of: ${themeList} — pick whichever best fits this brand's aesthetic and this post's mood",\n  "suggested_overlay_text": "a short punchy phrase (under 8 words) to overlay on the image IF this post genuinely benefits from on-image text, otherwise an empty string for a clean text-free image — most posts should get an empty string here, only use real overlay text when the post is making one sharp, standalone claim. Leave this an empty string whenever suggested_template is hard_truth_checklist -- that template has no photo to overlay text onto."${checklistFieldsBlock}`
    : ""
