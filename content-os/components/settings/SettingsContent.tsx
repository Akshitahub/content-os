...
import { useCreditTopup, type RazorpayResponse } from "@/hooks/useCreditTopup"
...
function PlanSection({ user, autoUpgradePlan }: { user: UserProps; autoUpgradePlan?: "agency" | null }) {
  ...
  // "Buy more credits" — single source of truth for the Razorpay top-up
  // purchase flow (see hooks/useCreditTopup.ts), shared with any other UI
  // that wants to sell a credit pack without duplicating the Razorpay
  // integration. Separate from the plan-upgrade flow above since a pack
  // purchase can happen alongside (not instead of) a plan change.
  const {
    topupConfirming,
    setTopupConfirming,
    topupState,
